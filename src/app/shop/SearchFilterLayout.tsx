...
export default function SearchFilterLayout({ children, collections }: SearchFilterLayoutProps) {
  ...
  const [optimisticFilters, setOptimisticCollectionIds] = useOptimistic(
  ...
  function updateFilters(updates: Partial<typeof optimisticFilters>) {
    ...
    startTransition(() => {
      ...
      setOptimisticCollectionIds(newState);
    ...
    <main className="group flex flex-col items-center justify-center gap-10 px-5 py-10 lg:flex-row lg:items-start">
      ...
      >
        ...
        <PriceFilter
          ...
          updatePriceRange={(min, max) => updateFilters({ price_min: min, price_max: max })} />
      </aside>
          ...
          return (
            <li key={collectionId}>
              <label className="flex cursor-pointer items-center gap-2 font-medium">
                <Checkbox
                  id={collectionId}
                  checked={selectedCollectionIds.includes(collectionId)} onCheckedChange={(checked) => {
                    updateCollectionIds(
                      checked
...
interface SortedFilterProps {
  sort: string | undefined,
  updateSort: (value: ProductsSort) => void
}

function SortedFilter({ sort, updateSort }: SortedFilterProps) {
  return (
